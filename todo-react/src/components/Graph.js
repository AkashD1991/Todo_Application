...
class graph extends Component {
    ...
    componentWillMount = () => {
        ...
        axios.get('http://localhost:4000/tasks/gettasks')
            .then(res => {
    ...
    sevendaysregitration = () => {
        ...
        var user = this.state.allUserDetails;
        user.map(users => {
            var Rdate = new Date(users.Regdate);
            this.calculate(users.Regdate);
        })
    ...
    calculate = (dateR) => {
        ...
        var Regdate = new Date(dateR);  // create a date object directly from `dob1` argument
        var age_now = today.getFullYear() - Regdate.getFullYear();
        var m = today.getMonth() - Regdate.getMonth();
        const noOfDay = Math.round(today.getTime() - Regdate.getTime()) / (one_day);
        if (noOfDay <= 7) {
